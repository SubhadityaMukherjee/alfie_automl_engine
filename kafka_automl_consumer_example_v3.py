...
logger.info(
    f"  AutoML Tabular: {AUTOML_TABULAR_URL} (host: {TABULAR_AUTOML_HOST}, port: {TABULAR_AUTOML_PORT})"
)
logger.info(
    ...
    try:
        ...
        if task_category == "tabular":
            ...
            try:
                ...
                logger.error(
                    f"Failed to connect to AutoML service at {AUTOML_TABULAR_URL}"
                )
                logger.error(f"   Error: {e}")
                logger.error(
...
async def run_consumer() -> None:
    ...
    logger.info(f"Data Warehouse API: {API_BASE} (host: {DW_HOST}, port: {DW_PORT})")
    logger.info(
        f"AutoML Tabular service: {AUTOML_TABULAR_URL} (host: {TABULAR_AUTOML_HOST}, port: {TABULAR_AUTOML_PORT})"
    )
