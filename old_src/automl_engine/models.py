...
class SessionState(BaseModel):
    ...
    jinja_environment: ClassVar[Environment] = Environment(
        loader=FileSystemLoader(Path("automl_engine/prompt_templates/"))
    )
    ...
    def get_all_messages_by_role(self, roles: List[str]) -> str:
        return "\n".join([m.content for m in self.messages if m.role in roles])
