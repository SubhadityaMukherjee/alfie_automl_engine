"""Integration tests for the audio AutoML router."""
...
@patch(
    "app.audio_automl.router.deployment_instructions",
    return_value="deploy instructions",
)
def test_deployment_instructions(mock_fn):
    resp = client.post("/automl/audio/deployment_instructions/")
    assert resp.status_code == 200
    assert "instructions" in resp.json()


@patch(
    "app.audio_automl.router.audio_data_instructions",
    return_value="data instructions",
)
def test_accepted_format(mock_fn):
    resp = client.post("/automl/audio/accepted_format/")
    assert resp.status_code == 200
    assert "instructions" in resp.json()


# ---------------------------------------------------------------------------
# find_best_model_for_audio — metadata errors
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# find_best_model_for_audio — validation errors
# ---------------------------------------------------------------------------


@patch("app.audio_automl.orchestrator.validate_audio_inputs")
@patch("app.audio_automl.orchestrator.extract_and_locate_dataset")
@patch("app.audio_automl.orchestrator.download_dataset")
@patch(
    "app.audio_automl.orchestrator.resolve_download_url",
    return_value="http://download",
)
@patch(
    "app.audio_automl.orchestrator.fetch_dataset_metadata",
...
def test_best_model_validation_error(
    ...
    mock_extract.return_value = (Path("/tmp/labels.csv"), Path("/tmp/audio"))
    mock_validate.return_value = "Audio directory not found: /tmp/audio"

    resp = client.post("/automl/audio/best_model/", data=_AUDIO_PARAMS)
    assert resp.status_code == 400
    assert "Audio directory" in resp.json()["error"]


@patch("app.audio_automl.orchestrator.validate_audio_inputs", return_value=None)
@patch("app.audio_automl.orchestrator.extract_and_locate_dataset")
@patch("app.audio_automl.orchestrator.download_dataset")
@patch(
    "app.audio_automl.orchestrator.resolve_download_url",
...
def test_best_model_unsupported_task_type(
    ...
    params = {**_AUDIO_PARAMS, "task_type": "text_classification"}
    resp = client.post("/automl/audio/best_model/", data=params)
    assert resp.status_code == 400
    assert "Unsupported task_type" in resp.json()["error"]


# ---------------------------------------------------------------------------
# find_best_model_for_audio — training errors
# ---------------------------------------------------------------------------


@patch("app.audio_automl.orchestrator.train_automl")
@patch("app.audio_automl.orchestrator.validate_audio_inputs", return_value=None)
@patch("app.audio_automl.orchestrator.extract_and_locate_dataset")
@patch("app.audio_automl.orchestrator.download_dataset")
@patch(
...
# ---------------------------------------------------------------------------
# find_best_model_for_audio — happy path
# ---------------------------------------------------------------------------
...
@patch(
    "app.audio_automl.orchestrator.convert_leaderboard_safely",
    return_value=({"score": 0.95}, "lb_str"),
)
@patch("app.audio_automl.orchestrator.serialize_and_zip_model")
@patch("app.audio_automl.orchestrator.train_automl", return_value=MagicMock())
@patch("app.audio_automl.orchestrator.validate_audio_inputs", return_value=None)
@patch("app.audio_automl.orchestrator.extract_and_locate_dataset")
@patch("app.audio_automl.orchestrator.download_dataset")
@patch(
