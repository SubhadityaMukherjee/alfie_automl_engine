...
def test_init_from_dataframe():
    df = _make_df()
    ds = ImageClassificationFromCSVDataset(csv_file=df, root_dir=Path("/irrelevant"), img_col="filename", label_col="label")
    assert len(ds) == 4
...
def test_init_from_csv_path(tmp_path):
    ...
    ds = ImageClassificationFromCSVDataset(csv_file=csv_path, root_dir=tmp_path, img_col="filename", label_col="label")
    assert len(ds) == 4
    assert ds.classes == ["cat", "dog"]
...
def test_len():
    df = _make_df()
    ds = ImageClassificationFromCSVDataset(csv_file=df, root_dir=Path("/x"), img_col="filename", label_col="label")
...
def test_integer_labels_in_dataframe():
    ...
    df = pd.DataFrame({"img": ["a.png", "b.png", "c.png", "d.png"], "lbl": [0, 0, 1, 1]})
    ds = ImageClassificationFromCSVDataset(csv_file=df, root_dir=Path("/x"), img_col="img", label_col="lbl")
    assert len(ds) == 4
