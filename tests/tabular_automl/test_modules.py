...
class TestAutoMLTrainer:
    def test_train_test_split_given_no_test(self, train_df):
        final_train_df, final_test_df = trainer.train_test_split(train_df=train_df, test_df=None)
        ...
        reconstructed = (
            pd.concat([final_train_df, final_test_df]).sort_index()
        )
        assert reconstructed.reset_index(drop=True).equals(train_df.sort_index().reset_index(drop=True))
    
    def test_train_test_split_given_test(self, train_df, test_df):
        final_train_df, final_test_df = trainer.train_test_split(train_df=train_df, test_df=test_df)
