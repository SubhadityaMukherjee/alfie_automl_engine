...
async def check_alt_text(image_url: str = Form(...), alt_text: str = Form(...)):
    ...
    try:
        ...
        return {"src": image_url, "alt_text": alt_text, "evaluation": result}
    except Exception as e:
...
async def check_accessibility(file: UploadFile = File(...)):
    ...
    async def process_chunk(i, chunk, start, end):
        async with sem:
            ...
            try:
                ...
                # Get response (non-streaming version)
                ...
                for src, alt in images:
                    try:
                        ...
                        error_msg = str(e)
                        image_feedback.append({"src": src, "alt_text": alt, "error": error_msg})
    ...
    tasks = [process_chunk(i, chunk, start, end) for i, (chunk, (start, end)) in enumerate(zip(chunks, ranges))]
