"""Orchestration of the audio AutoML training pipeline.

The router layer is intentionally thin: it binds HTTP form parameters, builds an
``AudioTrainingRequest``, delegates to ``run_audio_pipeline``, and translates
...
_AUDIO_SUCCESS_MESSAGE = (
    "Audio AutoML training completed successfully and model uploaded to AutoDW"
)
...
@dataclass(frozen=True)
class AudioTrainingResult:
    """Successful outcome of the audio pipeline."""
...
def _require_zip(metadata: dict) -> None:
    ...
    if metadata.get("file_type") != "zip":
        raise AutoMLValidationError("Audio AutoML requires a ZIP dataset.")
...
async def run_audio_pipeline(
    req: AudioTrainingRequest, *, task_id: str | None
) -> AudioTrainingResult:
    """Run the audio AutoML pipeline and upload the best model.
    ...
    with dataset_workspace(f"audio_automl_{req.dataset_id}") as workdir:
        # 4. Download & extract.
    ...
    logger.info("Audio AutoML training completed and model uploaded successfully.")
    return AudioTrainingResult(
        message=_AUDIO_SUCCESS_MESSAGE, leaderboard=leaderboard_str
    )
